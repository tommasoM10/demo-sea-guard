...
export class Tracker {
  ...
  update(detections, posesById, frameW, frameH){
    ...
    const dets = detections.filter(d => pointInPoly(centroid(d), roiPx));
    ...
    while (true){
      ...
      tr.bbox = dets[bj]; tr.lastSeen = t; tr.history.push({t, c: centroid(dets[bj])});
    ...
    for (let j=0;j<dets.length;j++){
      if (!assignedDet.has(j)){
        ...
        this.tracks.set(id, {id, bbox:d, state:'VISIBLE', created:t, lastSeen:t, lostSec:0, history:[{t,c:centroid(d)}], alerts:{pre:false,full:false}, poseRisk:0});
      }
    }

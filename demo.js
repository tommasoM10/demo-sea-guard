// Demo source: renders synthetic "people" over a water-like background and outputs a MediaStream
export class DemoSource {
  constructor(width=1280, height=720){
    ...
    this.events = [
      {at: 6, id: 1, action: 'submerge', dur: 7}, // person 2 "goes under" for 7s
    ];
    this.activeEvents = [];
  }
  ...
  step(dt=1/30){
    ...
    for (let i=0;i<this.people.length;i++){
      ...
      // draw simplified human (head + torso)
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.fillStyle = '#ffeeaa';
      ctx.beginPath(); // head
      ctx.arc(p.x, p.y + bob - p.h/2, 10, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = '#c0d8ff';
      ctx.fillRect(p.x- p.w/2, p.y + bob - p.h/2 + 12, p.w, p.h-12);
      ctx.restore();
    }
  }
}
